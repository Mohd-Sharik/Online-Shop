...
import com.jpa.test.shopService.TbOsEncryptionService;
...
public class TbOsEncryptionController {
	
	@Autowired
	TbOsEncryptionService encryptionService;
	
	@Autowired
	TbOsDbErrService dbErrService;
	
	
	private static final ILogger logger = LoggerFactory.getLogger(TbOsEncryptionController.class);
	
	
	
	@RequestMapping(value = "/getList", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ServiceOperationResult<List<TbOsEncryptioModel>> getList(HttpServletRequest request, @RequestBody FilterParameter filter)
	{
		
//		  String name = request.getUserPrincipal().getName();
//		  String path = request.getServletPath();
		
		ServiceOperationResult<List<TbOsEncryptioModel>> result = new ServiceOperationResult<List<TbOsEncryptioModel>>();
		try
		{
			result = this.encryptionService.getAllEncryption(filter);
